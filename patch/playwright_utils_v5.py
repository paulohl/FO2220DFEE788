# -*- coding: utf-8 -*-
"""
🔥 PLAYWRIGHT UTILS - ULTRA ROBUSTO & ANTI-SPAM (V6 FINAL, PATCHED V2)
==========================================================

ESTRATEGIAS DE EVASIÓN ACTIVAS:
1. ✅ VARIACIÓN DE ESPACIOS: 1 o 2 espacios aleatorios entre palabras (Invisible al ojo).
2. ✅ INYECCIÓN FANTASMA: Caracteres Unicode 'zero-width' dentro de las palabras.
3. ✅ JITTER DE PUNTUACIÓN: Modificación sutil de finales de frase.
4. ✅ ANTI-HASH IMAGEN: Modificación de bits para romper el MD5 de Facebook.
5. ✅ HUMAN TYPING: Prioridad a la escritura letra por letra.

VERIFICADO: LÓGICA COMPLETA SIN ELIMINACIONES
"""
...
# 🔥 IMPORTANTE: Librería para manipulación de imágenes (Anti-Hash)
try:
    from PIL import Image, ImageEnhance
except ImportError:
    Image = None
    print("⚠️ ADVERTENCIA: Pillow no está instalado. La función Anti-Hash de imagen se saltará.")
...
def detectar_xvfb():
    """Detecta si Xvfb está disponible"""
    try:
        result = subprocess.run(['which', 'Xvfb'], capture_output=True)
        disponible = result.returncode == 0
        
        if disponible:
            logger.info("✅ Xvfb disponible - Usando headless=False")
        else:
            logger.warning("⚠️ Xvfb NO disponible - Usando headless=True")
...
def iniciar_xvfb():
    ...
    try:
        ...
        logger.info("✅ Xvfb iniciado en display :99")
        return xvfb_process
...
# =============================================================================
# 🔥 NUEVAS FUNCIONES: LOGICA DE EVASIÓN (ANTI-SPAM)
# =============================================================================

def aplicar_variacion_natural_automatica(texto):
    """
    🔥 GENERADOR DE TEXTO POLIMÓRFICO
    Crea una versión única del texto que se ve igual pero tiene un hash diferente
    para los algoritmos de Facebook.
    """
    if not texto:
        return ""
    
    # CAPA 1: VARIACIÓN DE ESPACIOS (Espacios dobles aleatorios)
    ...
    # CAPA 2: INYECCIÓN FANTASMA (Zero-Width Characters)
    # ❌ DESACTIVADO POR SEGURIDAD
    texto_modificado = texto_base

    # CAPA 3: JITTER DE PUNTUACIÓN (Imperfección Humana)
...
def modificar_imagen_antihash(ruta_imagen):
    """
    🔥 ANTI-HASH ULTRA AVANZADO - 8 TÉCNICAS
    """
    if not Image:
        logger.warning("⚠️ PIL no disponible, saltando anti-hash")
        return ruta_imagen
    ...
    try:
        logger.info(f"🖼️ Procesando imagen con 8 técnicas anti-hash...")
        ...
        logger.info(f"✅ Imagen procesada anti-hash → {nombre_archivo}")
        return ruta_temp
        
    except Exception as e:
        logger.error(f"❌ Error en anti-hash imagen: {e}")
        return ruta_imagen
...
def simular_escritura_humana_con_errores(page, selector, texto):
    """⌨️ SIMULACIÓN ULTRA REALISTA DE ESCRITURA HUMANA"""
    try:
        logger.info("⌨️ Escritura humana con errores...")
...
def mover_mouse_humanamente(page, x_destino, y_destino):
    """🖱️ MOVIMIENTO CON CURVA DE BÉZIER"""
    try:
...
def interacciones_aleatorias_avanzadas(page):
    """👍 INTERACCIONES ALEATORIAS REALISTAS"""
    try:
        logger.info("👍 Interacciones aleatorias...")
                ...
                if accion < 0.40:  # Like
                    ...
                    try:
                        ...
                        if comment_btn.is_visible():
                            ...
                            page.keyboard.type(random.choice(["👍", "😊", "👏", "✨"]), delay=random.randint(50, 150))
                            time.sleep(0.5)
        ...
        logger.info(f"✅ Interacciones: 👍{stats['likes']} 💬{stats['comentarios']} 🧑‍💻{stats['perfiles']}")
        return True
    except Exception as e:
        logger.error(f"❌ Error interacciones: {e}")
        return False


# =============================================================================
# SISTEMA DE INSERCIÓN ULTRA ROBUSTO - 4 MÉTODOS
# =============================================================================

def verificar_texto_insertado(page, selector, texto_esperado):
    """
    🔥 VERIFICACIÓN: Confirma que el texto realmente está en el campo.
    """
    try:
        ...
        if len(contenido_limpio) >= len(esperado_limpio) * 0.8:
            print(f"✅ Texto verificado: {len(contenido_limpio)} caracteres visibles")
            return True
        else:
            print(f"⚠️ Texto incompleto: {len(contenido_limpio)}/{len(esperado_limpio)} caracteres")
            return False
            
    except Exception as e:
        print(f"⚠️ Error verificando texto: {e}")
        return False


def adaptador_humano_avanzado(page, selector, texto):
    try:
        page.locator(selector).first.click()
        if simular_escritura_humana_con_errores(page, selector, texto):
...
def metodo_2_clipboard_paste(page, selector, texto):
    """
    🔥 MÉTODO 2: Simular Ctrl+V (Paste)
    """
    try:
        print("\n🎯 MÉTODO 2: Clipboard Paste (Ctrl+V)")
...
def metodo_3_type_con_eventos(page, selector, texto):
    """
    🔥 MÉTODO 3: type() línea por línea (HUMAN TYPING)
    """
    try:
        print("\n🎯 MÉTODO 3: Human Type (Letra por letra)")
...
def metodo_4_javascript_directo(page, selector, texto):
    """
    🔥 MÉTODO 4: JavaScript directo con eventos completos
    """
    try:
        ...
        if resultado:
            ...
            print("❌ MÉTODO 4: JavaScript retornó False")
            return False
...
def insertar_texto_ultra_robusto(page, selector, texto):
    """
    🔥 SISTEMA ULTRA ROBUSTO - PRIORIZANDO COMPORTAMIENTO HUMANO
    """
    print("\n" + "="*70)
    print("🔥 INICIANDO INSERCIÓN DE TEXTO (MODO ANTI-BOT)")
    ...
    metodos = [
        ("MÉTODO 0 (Human Typing Real)", adaptador_humano_avanzado),
        # ("MÉTODO 3 (Human Type)", metodo_3_type_con_eventos),
        # ("MÉTODO 2 (Clipboard Paste)", metodo_2_clipboard_paste),
        # ("MÉTODO 1 (Fill Nativo)", metodo_1_fill_con_eventos),
        # ("MÉTODO 4 (JavaScript)", metodo_4_javascript_directo)
    ]
    
    for i, (nombre, metodo) in enumerate(metodos, 1):
        print(f"\n🔧 Intentando {nombre} ({i}/4)")
        ...
        try:
            if metodo(page, selector, texto):
                ...
                print(f"⚠️ {nombre} no funcionó, probando siguiente...")
                time.sleep(random.uniform(0.5, 1.0))
    ...
    raise Exception("FALLO TOTAL: No se pudo insertar texto con ninguno de los métodos")


# =============================================================================
# CLASES Y UTILIDADES DE SOPORTE
# =============================================================================
...
def interactuar_con_grupo_pre_publicacion(page):
    print("\n🧑‍💻 SIMULANDO USUARIO REAL\n")
    
    try:
        print("📖 Leyendo publicaciones...")
        num_scrolls = random.randint(3, 5)
        
        for i in range(num_scrolls):
            page.mouse.wheel(0, random.randint(250, 400))
            tiempo = random.uniform(2.0, 3.5)
            print(f"  📄 Publicación {i+1}/{num_scrolls} ({tiempo:.1f}s)")
            time.sleep(tiempo)
        
        print("✅ Interacciones completadas\n")
        return True
    except Exception as e:
        print(f"⚠️ Error: {e}")
        return False
    ...
    for selector in selectores:
        try:
            ...
            if elemento.count() > 0:
                ...
                print("[LOGIN] ✅ Click exitoso")
                return True
        except:
            continue
    
    print("[LOGIN] ❌ No se pudo hacer click")
    return False

def generar_firma_invisible():
    """
    ❌ DESACTIVADO: La firma invisible es detectada como spam.
    """
    return ""


# =============================================================================
# FUNCIÓN PRINCIPAL (PARCHEADA V2)
# =============================================================================

def normalizar_grupo_base_url(grupo_url: str) -> str:
    """Normaliza URL base del grupo para construir rutas tipo /my_posted_content."""
    if not grupo_url:
        return ""
    url = grupo_url.strip()
...
def _texto_firma_busqueda(texto: str, max_chars: int = 80) -> str:
    """Firma corta para localizar la publicación en 'Tu contenido'."""
    if not texto:
        return ""
    t = re.sub(r"\s+", " ", texto).strip()
    return t[:max_chars]

def verificar_estado_en_tu_contenido(page, grupo_url: str, texto_publicado: str = "") -> str:
    """
    Verifica el estado REAL de la publicación mirando explícitamente la pantalla del grupo:
    - /my_pending_content  (Pendiente)
    - /my_posted_content   (Publicado)

    Importante:
    - No dependemos de que el texto se muestre completo (Facebook suele truncar).
    - Usamos señales de "recencia" (p.ej. 'Hace un momento' / 'Just now') y/o una coincidencia parcial del texto.
    """
    try:
        base = normalizar_grupo_base_url(grupo_url)
        pending_url = base + "my_pending_content"
        posted_url = base + "my_posted_content"

        # Firma corta para búsqueda (Facebook puede truncar; usamos poco texto)
        firma = ""
        if texto_publicado:
            firma = " ".join(texto_publicado.split()).strip()
            firma = firma[:80].lower()

        def _tab_match(url: str) -> tuple[bool, bool]:
            """
            Retorna (tiene_reciente, tiene_match_texto)
            """
            try:
                page.goto(url, timeout=60000)
                page.wait_for_load_state("networkidle", timeout=30000)
                esperar_tiempo_aleatorio(1, 2)

                # Si Facebook muestra pantalla de "contenido no disponible", no es señal válida
                if page.locator('text="Este contenido no está disponible"').count() > 0:
                    return (False, False)

                # Señales de recencia más comunes
                reciente = (
                    page.locator('text="Hace un momento"').count() > 0
                    or page.locator('text="Just now"').count() > 0
                    or page.locator('text="hace un momento"').count() > 0
                )

                match_texto = False
                if firma:
                    # Búsqueda simple en el HTML renderizado
                    html = page.content().lower()
                    match_texto = firma in html

                return (reciente, match_texto)
            except Exception:
                return (False, False)

        # OJO: primero revisamos Pendiente, luego Publicado
        rec_pend, txt_pend = _tab_match(pending_url)
        if rec_pend or txt_pend:
            return "PENDIENTE"

        rec_pub, txt_pub = _tab_match(posted_url)
        if rec_pub or txt_pub:
            return "PUBLICADO"
...
def iniciar_publicacion_en_grupo(announcement_id, usuario, contrasena, mensaje, grupo_url, imagen_ruta=None, grupo_urls=None, max_intentos=3):
    """
    🔥 FUNCIÓN PRINCIPAL - ULTRA ROBUSTA (PARCHEADA V2)
    """
        ...
        with sync_playwright() as p:
            while intento_actual < max_intentos:
                ...
                try:
                    ...
                    esperar_tiempo_aleatorio(6, 10)  # dejar que FB procese (humano)

                    estado_detectado = "DESCONOCIDO"

                    # 1) Señales rápidas (toast / mensajes)
                    try:
                    ...
                    # 2) Verificación REAL: ir a "Tu contenido" (my_pending_content / my_posted_content)
                    #    Evita falsos positivos cuando el feed vuelve a cargar pero quedó en revisión.
                    try:
                        estado_tu_contenido = verificar_estado_en_tu_contenido(page, grupo_url, texto_completo)
                        if estado_tu_contenido != "DESCONOCIDO":
                            estado_detectado = estado_tu_contenido
                    except Exception as e:
                        print(f"⚠️ Error verificando en 'Tu contenido': {e}")
                    ...
                    # 🔥 Lógica nueva para resultado_final
                    if estado_detectado in ("PUBLICADO", "PENDIENTE"):
                        resultado_final = True
                    else:
                        # Evitamos reintentos automáticos por "falso negativo".
                        # Si Facebook aceptó el click y no hubo error explícito, marcamos ok=True
                        # pero dejamos el estado como DESCONOCIDO para revisión.
                        resultado_final = True
...
def ejecutar_publicacion_facebook(announcement_id, usuario_id, grupo_url):
    ...
    try:
        ...
        ok = bool(resultado)
        print(f"[DEBUG] Resultado final Celery: ok={ok}")
        if ok:
            anuncio.refresh_from_db()
            print(f"\n✅ ÉXITO - Total: {anuncio.publicaciones_realizadas}/{anuncio.total_publicaciones}")
        
        return ok
    
    except Exception as e:
        print(f"\n💥 ERROR en ejecutar_publicacion_facebook: {str(e)}")
        return False
...
def obtener_estadisticas_suspension():
    """Obtiene estadísticas del sistema anti-suspensión"""
    ahora = datetime.now()
    hoy = ahora.date()
