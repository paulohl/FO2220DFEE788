...
def verificar_estado_en_tu_contenido(page, grupo_url: str, texto_publicado: str, timeout_ms: int = 45000) -> str:
    """Verifica estado REAL visitando 'Tu contenido' del grupo.
    Retorna: PUBLICADO | PENDIENTE | DESCONOCIDO
    """
    base = normalizar_grupo_base_url(grupo_url)
    if not base:
        return "DESCONOCIDO"

    needle = _texto_firma_busqueda(texto_publicado)
    if not needle:
        return "DESCONOCIDO"

    url_posted = base + "my_posted_content"
    url_pending = base + "my_pending_content"

    def _buscar_en(url: str) -> bool:
    ...
    try:
        if _buscar_en(url_pending):
            return "PENDIENTE"
    except Exception:
        pass

    try:
        if _buscar_en(url_posted):
            return "PUBLICADO"
    except Exception:
        pass
