...
def normalizar_grupo_base_url(grupo_url: str) -> str:
    """Devuelve base URL tipo: https://www.facebook.com/groups/<id>/"""
    if not grupo_url:
        return ""
    m = re.search(r"(https?://www\.facebook\.com/groups/[^/?#]+)", grupo_url)
    base = m.group(1) if m else grupo_url
    base = base.rstrip("/") + "/"
    return base


def _texto_busqueda_corto(texto: str, max_len: int = 80) -> str:
    if not texto:
        return ""
    t = " ".join(texto.split())
...
def _post_contiene_texto(page, texto_busqueda: str, max_articulos: int = 6) -> bool:
    """Busca el texto (parcial) dentro de los artículos de 'Tu contenido'."""
    tb = _texto_busqueda_corto(texto_busqueda, 80).lower()
    if not tb or len(tb) < 12:
        return False

    try:
        arts = page.locator('div[role="article"]')
        n = min(arts.count(), max_articulos)
        for i in range(n):
            try:
                txt = arts.nth(i).inner_text(timeout=5000).lower()
                if tb in txt:
                    return True
            except Exception:
                continue
...
def verificar_estado_en_tu_contenido(page, grupo_url: str, texto_publicado: str) -> str:
    """Verifica estado real en la pantalla 'Tu contenido' del grupo.

    Devuelve: PUBLICADO | PENDIENTE | DESCONOCIDO
    """
    base = normalizar_grupo_base_url(grupo_url)
    if not base or "facebook.com/groups/" not in base:
        return "DESCONOCIDO"
    ...
    try:
        ...
        # Si el texto aparece en los últimos artículos, es Pendiente.
        if _post_contiene_texto(page, texto_publicado):
            return "PENDIENTE"
    except Exception:
        pass
    ...
    return "DESCONOCIDO"
                    ...
                    esperar_tiempo_aleatorio(5, 8)  # Esperar a que Facebook procese

                    estado_detectado = "DESCONOCIDO"

                    # Verificación principal: pantalla "Tu contenido" del grupo (fuente de verdad)
                    try:
                        estado_detectado = verificar_estado_en_tu_contenido(page, grupo_url, texto_completo)
                    except Exception as e:
                        print(f"⚠️ Error verificando en 'Tu contenido': {e}")
                        estado_detectado = "DESCONOCIDO"

                    # Fallback: indicadores básicos en la misma página (por si el post no incluye texto o FB aún no refleja en 'Tu contenido')
                    if estado_detectado == "DESCONOCIDO":
                        try:
                            ...
                            if estado_detectado == "DESCONOCIDO":
                                # Si volvimos al feed / aparece "Hace un momento", lo tratamos como publicado
                                indicadores_exito = [
                            ...
                            if estado_detectado == "DESCONOCIDO" and page.locator('div[role="dialog"]').count() > 0:
                                # Si seguimos en el cuadro, probablemente no terminó bien
                                print("⚠️ Seguimos en el cuadro de diálogo tras intentar publicar (posible fallo/validación).")
                        except Exception:
                            pass
            print("⚠️ Seguimos en el cuadro de diálogo tras intentar publicar (posible fallo/validación).")
    except Exception:
