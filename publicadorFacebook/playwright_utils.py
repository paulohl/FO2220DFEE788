# -*- coding: utf-8 -*-
"""
🔥 PLAYWRIGHT UTILS - ULTRA ROBUSTO & ANTI-SPAM (V6 FINAL)
==========================================================
...
def aplicar_variacion_natural_automatica(texto):
    ...
    for i, palabra in enumerate(palabras):
        ...
        if i < len(palabras) - 1:
            # 🔥 AJUSTE: Bajamos de 30% a 8% para evitar detección de "spinning"
            # HTML renderiza doble espacio como uno, pero el análisis de texto raw lo detecta
    ...
    # ---------------------------------------------------------
    # ❌ DESACTIVADO POR SEGURIDAD (Facebook detecta esto como spam)
    # chars_invisibles = ['\u200B', '\u200C', '\u200D', '\u2060']
    # ... (código desactivado) ...
    
    texto_modificado = texto_base  # Saltamos Capa 2
...
def modificar_imagen_antihash(ruta_imagen):
    ...
    1. ✅ Brillo imperceptible (±1-3%)
    2. ✅ Contraste imperceptible (±1-2%)
    3. ✅ Saturación imperceptible (±1-2%)
    4. ✅ Recorte microscópico (1-2 pixels)
    5. ✅ Modificación de píxeles aleatorios (3-5 píxeles)
    6. ✅ Calidad JPEG variable (92-98)
    7. ✅ Conversión RGB
    8. ✅ Eliminación EXIF automática
    """
    if not Image:
    ...
    try:
        ...
        # TÉCNICA 6: Calidad JPEG variable (92-98)
        quality_val = random.randint(92, 98)
        
        # TÉCNICA 7 y 8: Nombre único + Eliminación EXIF
        ...
        logger.info(f"✅ Imagen procesada: 8 técnicas aplicadas → {nombre_archivo}")
        return ruta_temp
...
# =============================================================================
# 🔥 COMPORTAMIENTO HUMANO AVANZADO (5 FUNCIONES)
# =============================================================================
...
def verificar_texto_insertado(page, selector, texto_esperado):
    ...
    NOTA: Limpiamos los caracteres invisibles antes de comparar para que la validación pase.
    """
    try:
        ...
        # Función para limpiar invisibles y normalizar espacios para la comparación lógica
        def limpiar_para_validacion(t):
            # Eliminar caracteres zero-width
            t = re.sub(r'[\u200B\u200C\u200D\u2060]', '', t)
            # Normalizar espacios dobles a simples para la comparación
            t = re.sub(r'\s+', ' ', t)
            return t.strip()
...
def metodo_3_type_con_eventos(page, selector, texto):
    ...
    Este es el método preferido para evitar SPAM.
    """
    try:
    ...
    try:
        ...
        resultado = page.evaluate(f"""
            () => {{
                ...
                // Insertar texto con HTML para saltos de línea
...
def insertar_texto_ultra_robusto(page, selector, texto):
    ...
    # 🔥 CAMBIO DE PRIORIDAD: Humano primero
    # 1. Type (Más humano, evita filtros de spam)
    ...
    raise Exception(f"FALLO TOTAL: No se pudo insertar texto con ninguno de los 4 métodos")
...
def generar_firma_invisible():
    ...
    Retorna string vacío para compatibilidad.
    """
    return ""
...
def iniciar_publicacion_en_grupo(announcement_id, usuario, contrasena, mensaje, grupo_url, imagen_ruta=None, grupo_urls=None, max_intentos=3):
    ...
    # ❌ FIRMA INVISIBLE DESACTIVADA (Riesgo de spam)
    # firma_invisible = generar_firma_invisible()
    texto_completo = mensaje_variado # + " " + firma_invisible
        ...
        with sync_playwright() as p:
            while intento_actual < max_intentos:
                ...
                try:
                    ...
                    for selector in selectores_texto:
                        try:
                            if page.locator(selector).count() > 0:
                                ...
                                # 🔥 LLAMADA AL SISTEMA ULTRA ROBUSTO
                                # Usamos el texto modificado aquí
                    ...
                    # ---------------------------------------------------------
                    print("\n👁️ Verificando estado de publicación...")
                    ...
                    try:
                        ...
                        indicadores_exito = [
                            ...
                            'text="Just now"', 
                            'text="Te damos la bienvenida"', # A veces sale esto tras publicar
                            'div[role="feed"]' # Si volvemos al feed
                        ]
                        
                        for ind in indicadores_exito:
                            if page.locator(ind).count() > 0:
                                estado_detectado = "PUBLICADO"
                                print(f"✅ Detectado: {ind}")
                                break
                        ...
                        if estado_detectado == "DESCONOCIDO":
                            ...
                            for ind in indicadores_pendiente:
                                if page.locator(ind).count() > 0:
                                    ...
                                    print(f"⚠️ Detectado: {ind}")
                                    break
                                    
                        # 3. Verificar si seguimos en el cuadro (FALLO)
                        if estado_detectado == "DESCONOCIDO":
                            if page.locator('div[role="dialog"]').count() > 0:
                                print("❌ Seguimos en el cuadro de diálogo")
                                # Intentar cerrar o detectar error
                    ...
                    try: page.screenshot(path=f"capturas/paso5_estado_{estado_detectado}.png")
                    except: pass
...
def ejecutar_publicacion_facebook(announcement_id, usuario_id, grupo_url):
    ...
    try:
        ...
        if resultado:
            # El contador se incrementa en tasks.py, no aquí
            anuncio.refresh_from_db()  # ✅ Refrescar para obtener el valor actualizado
